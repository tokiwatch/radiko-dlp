...
| `--apply` | Actually install the result into your crontab. Without it, the tool only **prints** what the crontab would look like and changes nothing |
...
# 2. Preview: prints the whole crontab as it would be. Nothing is changed
./radiko-cron
...
- **Preview by default.** Nothing is installed unless you pass `--apply`.
- **A different config file is refused.** The crontab holds a single managed block. If it was generated from another config file, `--apply` stops instead of replacing it:
  ...
  This protects your real schedule when you try another file such as `sample.toml`. Add `--force` only when you really want to replace it. A preview (without `--apply`) prints the same message as a warning.
